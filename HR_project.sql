...
SELECT job_id FROM jobs WHERE job_title LIKE "%Clerk%";
SELECT hire_date,concat(first_name," ",last_name) as Name  ,  job_id FROM employees WHERE job_id=13 OR job_id=17 OR job_id=18;

/*5. Create a query to display all the data from the EMP table. Separate each column by a comma. Name the column THE OUTPUT*/
SELECT concat_ws(",",employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id)as "THE OUTPUT" from employees;
...
SELECT concat(first_name," ",last_name) as Name,jobs.job_title FROM employees,jobs WHERE employees.manager_id="";
...
SELECT employees.last_name,jobs.job_title,employees.salary FROM employees,jobs WHERE jobs.job_title LIKE "%sales representative%" or jobs.job_title LIKE "%stock clerk%" AND employees.salary!= 2500 AND employees.salary!=3500 AND employees.salary!=5000;
